...
def combineJSON(path):
    ...
    output_path = f"Assignments/Assignment_5/nodePositions/Overall/{file_name}.json"
...
def getClusterBoxCoords(clusters):
    ...
    for cluster in clusters:
        ...
        ellipses.append(Ellipse(cluster.name, width+30, height+10, x_mean, y_mean, angle))
...
def drawGraph(G, clusters, ellipses, circlesize=100, fontsize=6):
    ...
    for idx, cluster in enumerate(clusters):
        ...
        for node, position in zip(cluster.nodes, cluster.coordinates):
            plt.scatter(position[0], position[1], color=colors[idx], zorder=3, s=circlesize)
    ...
    for ellipse in ellipses:
        ...
        plt.plot(l, r, 'darkorange', zorder=4)
...
if __name__ == "__main__":
    ...
    # path = combineJSON("Assignments/Assignment_5/nodePositions/newBlogosphere")

    # path = "Assignments/Assignment_5/nodePositions/Overall/ArgumentationNetwork.json"
